...
set.seed(500) # set sampling seed


# Define function for calculating model fit indices
...
# Define function to extract province random effects from the best fit model
...
#------------------------------------------------------------------------
# Define function to carry out posterior simulations from INLA results
####----POSTERIOR SIMULATIONS-----------------------------------
simPop <- function(model, dat, Aprediction, run)
{
  fixedeff  <- dens_hat <- pop_hat <- matrix(0, nrow=nrow(dat), ncol = run)
  #inla.seed = as.integer(runif(1)*.Machine$integer.max) # simulates random seeds
  inla.seed =  1657687559
  set.seed(inla.seed)
  print(inla.seed) # prints the random seed used
  m1.samp <- inla.posterior.sample(run, model, seed = inla.seed ,selection=list(x2=1, x3=1, x4=1,
                                                                                x5=1, x6=1),num.threads="1:1")
  
  sfield_nodes_mean <- model$summary.random$spatial.field['mean'] #  extract spatial random effects from mesh nodes
  ...
  for(i in 1:run)
  {
    fixedeff[,i] <-  
      ...
      dat$prov_ranef +              # province level random effects 
      model$summary.random$IDsp$mean + # cluster level random effects
      
      field_mean[,1] # spatially varying random effects
    
    dens_hat[,i]<- exp(fixedeff[,i]) # predicted density per draw 
    pop_hat[,i] <- dens_hat[,i]*dat$bldp # predicted population count per draw
  }
  
  # Calculate key statistics 
  mean_dens_hat <- apply(dens_hat, 1, mean, na.rm=T) # mean density 
  mean_pop_hat <- apply(pop_hat, 1, mean, na.rm=T) # mean count
  median_pop_hat <- apply(pop_hat, 1, quantile, probs=c(0.5), na.rm=T) # Median
  lower_pop_hat <- apply(pop_hat, 1, quantile, probs=c(0.025), na.rm=T) # lower bound
  upper_pop_hat <- apply(pop_hat, 1, quantile, probs=c(0.975), na.rm=T) # upper bound
  sd_pop_hat <- apply(pop_hat, 1, sd, na.rm=T) # standard deviation
  uncert_pop_hat <- (upper_pop_hat - lower_pop_hat)/mean_pop_hat# uncertainty
...
# ZONAL STATS
#---------------------------------
# Aggregate to National total population and calculate the uncertainties 
...
#  FUNCTION: Aggregate to CU total population and calculating uncertainties  
...
# SPDE OBJECT
#  spde parameters 
r0 <- 0.3 # range of spatil dependence 
nu <- 1 # smoothness parameter 
sigma0 <- 1 # marginal variance 
kappa0 <- sqrt(8*nu)/r0 # scale parameter 
tau0 <- 1/(sqrt(4*pi)*kappa0*sigma0) # variance parameter 

# simulate spde object 
...
# Simulate the Gaussian Markov Random Fields (GMRF)
...
# Specify the province and settlement type structures 
n.province <- 24 # 24 provinces 
n.set_typ <- 3 # 3 settlement types as in the real data 
n.sample <- nrow(dat.all)

# Simulate the number of provinces in the CU level data

# Equal number of CUs per province 
...
# Unequal number of provinces per CU
prop.prov <- c(0.037, 0.031, 0.053, 0.031, 0.049, 0.105, 0.032, 0.018, 0.068,0.036,
               ...
               0.023, 0.037, 0.028, 0.042) # these are the assumed proportion of the number of CUs in each province 

# Unequal number of settlement type per CU 
prop.set_typ <- c(0.084, 0.8, 0.116) # assumed proportion of CUs in each settlement type
...
###---Add the newly defined variables into the dataset 
...
dim(zpred <- covs) # 5 covariates are simulated for this study but more or less could be simulated. 
dim(dat.all)
...
# Simulate nugget effects and building counts per CU 
...
###---Simulate nugget and population counts per CU 
# Simulate 
sigma_e <- 0.15
...
fit_bld_dens <- function(dat, cover, A, iset, result_path1, pm)
{
  ...
  { 
    ...
    if(bm < 1) # partially observed settlement data/building counts
    {
      dat$bldm <- dat$bld
...
# RUN THE fit_bld_dens function for each pair of survey coverage and proportion of settlement data observed
coverp <- c(1, 0.8, 0.6, 0.4, 0.2)# Survey coverage 
coverb <- c(1, 0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65) # settlement proportion observed
for(i in 1:length(coverp))
{
  ...
  if (file.exists(result_path1)){
    ...
    dir.create(file.path(result_path1)) # create new directory 
    setwd(file.path(result_path1))
  }
  ...
  if(pm == 1){
    ...
    fit_bld_dens(ddat, coverb, A, iset, result_path1, pm=1) # implement fit_bld_dens function
    
  }
  if(pm < 1)
  {
    ind.mpop <- sample(nrow(ddat), pm*nrow(ddat))#prportion of the data missing
    ddat$popm <- ddat$pop
    ddat$popm[-ind.mpop] = NA#--set all missing pop to NA to allow prediction
